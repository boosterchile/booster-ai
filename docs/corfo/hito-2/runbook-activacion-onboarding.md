...
- [ ] **(c) La advertencia original se mantiene**: no aprobar solicitudes reales antes del paso 5 (flip) de este runbook — con el flag admin-provisioned OFF, el approve toma silenciosamente el modo legacy (precrea fila `users`, sin token ni link). Precisión sobre qué cierra (a): `SIGNUP_REQUEST_FLOW_ACTIVATED=false` congela SOLO los endpoints admin (listar/approve/reject → 503) — el `POST /api/v1/signup-request` público NO está gateado por diseño (spec §7.5/R6) y la página `/solicitar-acceso` + el link del login siguen vivos tras el merge: **las solicitudes seguirán acumulándose con 202 durante toda la ventana pre-flip** (esperable; no es un incidente). Lo que el flag en `false` garantiza es que nadie puede aprobarlas en modo legacy. No reabrir el flag manualmente antes del paso 5.
