...
- **Impacto**: el carbono **medido** (`exacto_canbus`) queda **sin insumo** — su deuda pasa de "cablear el
  ...
  funcional. **#624 NO se ve afectado** (distancia real = GPS + Routes, no CAN) → sigue mergeable.
...
El PO decidió sobre los 8: **los 8 originales quedaron CERRADOS sin merge**; los 6 rescatables reviven como PR fresco (rebase/rehecho desde main), los 2 irrescatables se cerraron definitivamente. De los 6 sucesores, **#630 y #626 están mergeados**; los otros 4 siguen abiertos esperando merge del PO.
