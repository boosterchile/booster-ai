...
|---|---|---|---|
| **#598** distancia real híbrida | RESCATAR | **#624** ✅ **MERGEADO** (`44c24a0`) | migr **0053** (en repo, **no** en prod hasta el deploy); draft/paso-1 + gate PO para el backfill |
| **#511** consumer alerta safety-p0 | RESCATAR | **#626** ✅ **MERGEADO** (`cd1f0ed`) | corrige el `documentation` de la alert policy (consumer real = `apps/api · POST /internal/safety-events (PUSH)`, no el skeleton `notification-service`). ⚠️ **el apply sigue PENDIENTE** — la política en GCP no se toca desde 2026-06-15, así que la alerta viva aún nombra el consumer incorrecto |
