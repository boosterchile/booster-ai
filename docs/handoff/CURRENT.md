...
### Triage read-only de los 8 PRs rezagados (6 agentes de verificación; reporte en `~/Downloads/triage-8-prs-2026-07-24.md`)
La decisión de cada uno es del PO — no se cerró/mergeó ninguno (salvo #598, cerrado a favor de #624).

| PR | Estado | Veredicto | Nota |
|---|---|---|---|
| **#624** (ex-#598) distancia real híbrida | MERGEABLE | **RESCATAR** | ya rebaseado; #598 cerrado |
| **#511** consumer alerta safety-p0 | MERGEABLE | **RESCATAR** | bug vivo (`telemetry-monitoring.tf:638` label = skeleton); doc-only, requiere apply del owner |
| **#526** hardening Secret Manager (INC-2026-06-19) | MERGEABLE | **RESCATAR** | mount directo de placeholder-validado-por-formato sigue abierto; solo entró el script suelto (#504); gate PO + apply |
| **#513** reconnect chat SSE | MERGEABLE | **RESCATAR** | loop 401/403 vivo (`use-chat-stream.ts`); flag: bundlea un fix web (SSE) + uno de api (rate-limit-pin) bajo título `fix(web)` |
| **#596** desacopla SLOs/monitoring | MERGEABLE | **RESCATAR** | enabler Fase A; `slo.tf` aún acopla por `module.service_*`; trivial (update-branch) |
| **#516** booleanFlag de @booster-ai/config | CONFLICTING | **REHACER** | core (~5 líneas) sigue válido, pero `release.yml` fue reescrito a dispatch-only y 2 followups ya cerraron aparte → rehacer fresco |
| **#426** sitio público + /signup (ADR-067) | CONFLICTING | **CERRAR** | el `/signup` gateado YA vive en main vía `/solicitar-acceso` + backend SEC-001; solo falta el sitio de contenido `apps/marketing` (build fresco, no rebase) — preservar `.specs/` + ADR-067 |
| **#256** UI cards reales (T11) | base fantasma | **CERRAR** | D11 v1 BLOQUEADO→v2 (#253/#258/#265/#267); apilado sobre #255 (cerrado sin merge); deps ausentes; diff real vs main = 1157 archivos que revertirían trabajo vivo |

**Infra (#511/#526/#596):** conviene agruparlos en una sola ventana de `terraform apply` del owner post-merge. Memorias: [[trivy-gate-severity-unset-2026-07]], [[plfl57-itinerario-vs-carga-sintetica-2026-07]], [[capa2-historial-traza-carga-gap-2026-07]].
