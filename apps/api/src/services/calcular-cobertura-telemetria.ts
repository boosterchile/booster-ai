 ...
 * ordenados asc por timestamp y filtrados de rows sin fix (lat/lng null).
 *
...
export async function cargarPingsVentana(opts: {
  ...
  for (const p of rows) {
    // Defensa contra rows malformados (schema permite lat/lng null sin GPS fix).
    if (p.lat === null || p.lng === null) {
      continue;
    }
    pings.push({ tMs: p.ts.getTime(), lat: Number(p.lat), lng: Number(p.lng) });
  }
  return pings;
}
