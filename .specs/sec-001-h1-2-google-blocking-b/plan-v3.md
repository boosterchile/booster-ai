...
- **Status**: **CERRADO 2026-09-13 — abandonado en T8; superado por `.specs/sec-001-h1-2-google-boundary-closure/` (ADR-057). Sin trabajo pendiente.** · Estado previo: Draft v3
- **Linked**:
  - Umbrella: [`../sec-001-h1-2-google-blocking/spec.md`](../sec-001-h1-2-google-blocking/spec.md).
  - DA history (cumulative) of THIS plan: [`./plan-review.md`](./plan-review.md) (v1: F-B1..F-B12 5P0+4P1+3P2 → v2: N-B1..N-B5 2P0+3P1).
...
### T7b: Inter-apply deploy-gate CI workflow (WIF auth per N-B1 fix)

- **Files**:
  - `.github/workflows/sprint-2c-b-deploy-gate.yml` (NEW, ~55 LOC; +5 for WIF auth block per N-B1).
- **LOC estimate**: ~55.
- **Depends on**: T7 merged + WIF_SERVICE_ACCOUNT_DEPLOY has `roles/cloudfunctions.viewer` (verify pre-merge).
- **Acceptance** (N-B1 fix):
  - **WIF auth pattern** per `release.yml` precedent:
    ...
    ```
  - **IAM grant verification** (pre-T7b apply or merge): `gcloud projects get-iam-policy booster-ai-494222 --flatten='bindings[].members' --filter='bindings.members:WIF_SA_EMAIL AND bindings.role:roles/cloudfunctions.viewer'` returns at least one binding. If absent, plan T7b acceptance includes adding the grant via terraform (NEW in `infrastructure/iam-deploy-sa.tf` or equivalent existing file).
...
- **Acceptance** (N-B2 fix — PATCH-first sequencing):
  ...
  - **Step 2 — compute new list (current minus 3 contexts to remove)**:
    ...
    ```
  - **Step 3 — PUT the new list** (PATCH array semantics in GitHub API requires PUT of full required_status_checks object):
    ```bash
    gh api -X PUT repos/boosterchile/booster-ai/branches/main/protection/required_status_checks \
      --field 'strict=true' \
      --field-from-file 'contexts=after.txt'
    ```
  - **Step 4 — verify**:
    ```bash
    gh api repos/boosterchile/booster-ai/branches/main/protection/required_status_checks \
...
- [x] DA v1 findings F-B1..F-B12 each fixed in v2 per prior §"What changed v1 → v2" table.
- [x] DA v2 findings N-B1..N-B5 each fixed in v3 per §"What changed v2 → v3" table above.
- [ ] DA v3 pass output captured: PENDING (this DA pass).
...
- **2026-05-27 18:30Z** — Plan v3 drafted addressing all N-B1..N-B5. Status: Draft v3 awaiting DA v3 pass + user approval.
