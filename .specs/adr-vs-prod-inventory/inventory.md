...
- **Findings 🔴 de seguridad del cierre (051-054) — YA TRACKEADOS, encuadre corregido post-cierre:**
  - **ADR-052/054**: el inventario los marcó como residuales urgentes, pero **el contexto SEC-001 los matiza** (los agentes no lo vieron): (1) el vector explotable está **CERRADO** (`EMPRESA_SELF_ONBOARDING_ENABLED=false` → `/empresas/onboarding` 403, hotfix `sec-001-empresa-onboarding-gate-hotfix`); (2) la blocking function OFFLINE es **DELIBERADA** — PO eligió **Alternativa G** (API-boundary 404 `user_not_registered` + reaper), dirección blocking-function superseded. El trabajo abierto vive en `sec-001-h1-2-google-boundary-closure` (Draft, espera PO) + `_followups/onboarding-flow-redesign.md` (P1).
  - **Net-new genuino** (no cubierto antes): `login.tsx:141-147` signup email/password client-side huérfano (vs `disabledUserSignup`); ADR-054 necesita amendment (enfoque abandonado + dato `firebase-functions@^3.x` falso). Consolidado en `_followups/signup-residual-consolidation-adr-052-054.md`.
- **Follow-ups dejados (2026-06-03):** `_followups/adr-020-supersede-gitlab-to-github-actions.md` · `_followups/adr-049-claude-md-settings-json-reconcile.md` · `_followups/signup-residual-consolidation-adr-052-054.md`.
