...
- [ ] `grep -n "OLD_DEMO_UIDS" apps/api/src` **no devuelve ningún literal de UID** (la lista viene de env validada). Concretamente: `grep -REn "nQSqGqVCHGUn8yrU21uFtnLvaCK2|Uxa37UZPAEPWPYEhjjG772ELOiI2|s1qSYAUJZcUtjGu4Pg2wjcjgd2o1|Gg9k3gIPa1cJZtgKC0RRkWQ0QHJ3" apps packages` → **0 resultados en código vivo** (`apps/api/src/**`, `apps/api/scripts/**`); `dist/` y los `.specs/`/`docs/adr/` históricos quedan fuera (build artefact + historia, no código fuente).
