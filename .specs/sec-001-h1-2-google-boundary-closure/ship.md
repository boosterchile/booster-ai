...
- **SHIPPED 2026-06-05**: PR #402 squash-merged a `main` (`d867bdf`, 16:11 UTC, 20/20 checks) → deploy aprobado en Environment `production` → Cloud Build `d61e54bc` **SUCCESS** (canary→100%, rev sirviendo imagen `sha256:77579e88…`) → **`terraform apply`** aplicado (scheduler reaper **paused** + log-metric + alert policy) → **1er tick dry-run validado** (`reaper.run.summary destructive=false, scanned=14, skip=14, disable=0`). Verificación post-deploy sana (0 errores/5xx). SC-1.2.2 Google leg = **MET**. _[Nota 2026-06-05: el `terraform apply` fue **targeted**; el decomiso de la blocking-fn quedó pendiente — ver banner en §"Cierre del ship". No afecta el veredicto de seguridad.]_

## Cierre del ship (2026-06-05)

> **⚠️ CORRECCIÓN 2026-06-05 (post-investigación de drift)** — Ver [`post-ship-drift-investigation.md`](./post-ship-drift-investigation.md).
> Este cierre afirmaba que el **decomiso de la blocking-fn ya estaba hecho** "porque no apareció en el plan". Eso fue un **falso negativo de `terraform apply -target`**: un plan/apply targeteado NO lista los recursos no targeteados. Un `terraform plan` completo (2026-06-05) prueba que el decomiso **nunca se aplicó** a prod: `google_cloudfunctions_function.before_create` (estado FAILED), `google_storage_bucket.auth_blocking_source` y su object siguen en el state, pendientes de DELETE. Tampoco se aplicó `google_logging_metric.auth_is_demo_blocked` + su alert (T4, observabilidad).
> **Impacto en seguridad: ninguno.** La blocking-fn nunca fue el control (abandonada pre-prod, IdP `blocking_functions=[]`), está FAILED y sin cablear. SC-1.2.2 = MET sigue siendo correcto en sustancia (boundary + self-onboarding OFF + reaper SÍ están en prod). Lo pendiente es **higiene + observabilidad**, a reconciliar con un `terraform apply -target` explícito de esos recursos, separado del drift de IAM Owner.

- **Deploy**: Cloud Build `d61e54bc` SUCCESS (~38 min; el GHA run #437 figuró `cancelled` por timeout 30m < canary — falso negativo, ver follow-up `release-deploy-job-timeout-vs-canary.md`). Rev Cloud Run sirviendo imagen `sha256:77579e88…` (= `db0c00b`), 100% tráfico, 0 errores/5xx.
- **Infra (terraform apply, targeted, por mí con OK del PO)**: `google_logging_metric.reaper_account_reaped` (DELTA) + `google_monitoring_alert_policy.reaper_volume_anomaly` (activa) + `google_cloud_scheduler_job.reap_inert_idp_accounts` (**PAUSED**, diario 04:00 Santiago, dry-run). El cambio colateral del Cloud Run (`template.revision → null`, por `depends_on=module.service_api`) creó una revisión nueva con **imagen idéntica** → redeploy no-op, sin regresión (`image` en `ignore_changes`, ADR-013). ~~El decomiso de la blocking-fn ya estaba hecho (no apareció en el plan).~~ **[CORREGIDO 2026-06-05: FALSO — falso negativo de `-target`; el decomiso NUNCA se aplicó. Ver banner arriba + `post-ship-drift-investigation.md`.]** Drift no relacionado catalogado en follow-up `terraform-tfvars-example-stale.md` (NO bundleado).
- **1er tick dry-run (validado)**: resume→run→re-pause. `reaper.run.summary`: `destructive=false, scanned=14, skip=14, disable=0, delete=0, wait=0`. Cero acciones; scheduler re-pausado; métrica en 0; alerta sin disparar.
- **Pendiente (post-ship, fuera de scope)**: gate de 1er run destructivo (§"Gate de 1er run destructivo") antes de `REAPER_DESTRUCTIVE=true`; despausar el scheduler cuando el PO decida operar el reaper de forma recurrente.
...
- [x] **9. Config/secrets** — Sin secretos nuevos. `REAPER_DESTRUCTIVE` no necesita setearse (default OFF). El scheduler reusa el SA `internal-cron-invoker`; el runtime SA del api ya tiene `firebaseauth.admin`. ✅ **`terraform apply` (targeted) aplicado 2026-06-05** (scheduler paused + metric reaper + alert). **[CORREGIDO 2026-06-05: el decomiso de la blocking-fn NO se aplicó — falso negativo de `-target`. Pendiente. Ver §"Cierre del ship" banner.]** Ver §"Cierre del ship".
