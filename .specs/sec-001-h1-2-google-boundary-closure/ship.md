...
- **SHIPPED 2026-06-05**: PR #402 squash-merged a `main` (`d867bdf`, 16:11 UTC, 20/20 checks) → deploy aprobado en Environment `production` → Cloud Build `d61e54bc` **SUCCESS** (canary→100%, rev sirviendo imagen `sha256:77579e88…`) → **`terraform apply`** aplicado (scheduler reaper **paused** + log-metric + alert policy) → **1er tick dry-run validado** (`reaper.run.summary destructive=false, scanned=14, skip=14, disable=0`). Verificación post-deploy sana (0 errores/5xx). SC-1.2.2 Google leg = **MET**.
...
- [x] **9. Config/secrets** — Sin secretos nuevos. `REAPER_DESTRUCTIVE` no necesita setearse (default OFF). El scheduler reusa el SA `internal-cron-invoker`; el runtime SA del api ya tiene `firebaseauth.admin`. ✅ **`terraform apply` aplicado 2026-06-05** (scheduler paused + metric + alert; decomiso ya estaba hecho). Ver §"Cierre del ship".
