...
- [x] **SC-G8 (cierre del residual — gated P0-1)** ✅ 2026-06-04: `sec-001-cierre` §3 SC-1.2.2 transicionó `TRACKED_RESIDUAL → MET` (amendment A4). Condiciones cumplidas: self-serve onboarding OFF (verificado, `route-boundary-audit.md`) + SC-G1 audit sin GAP + SC-G1b harness activo (T2) + reaper desplegado (T9, dry-run; destructivo gateado). "MET" = no queda path self-signup→activa abierto. Followup `sprint-2c-google-blocking-function.md` cerrado/superseded con puntero. CURRENT.md actualizado.
