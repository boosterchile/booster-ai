...
- [ ] **SC-1.2.2** _(v3.4 amendment A3 2026-05-25: partial-cierre split — email/password leg in-scope Sprint 2b; Google leg TRACKED_RESIDUAL Sprint 2c)_: Identity Platform sign-up disabled split en 2 legs:
  ...
  - **Google leg (Sprint 2c TRACKED_RESIDUAL)**: backend gate require Firebase Auth Blocking Function `beforeCreate` (Cloud Function separada) que rechaza first sign-in si no hay `solicitudes_registro.estado=aprobado` matching email. **Rationale defer**: el repo no tiene `auth-google-callback.ts` route (verificado 2026-05-25: `apps/web/src/hooks/use-auth.ts:85` usa `signInWithPopup(firebaseAuth, googleProvider)` 100% client-side; Identity Platform GA no expone toggle per-provider "allow new accounts to sign up" para Google); implementar Blocking Function merece spec/ADR propio. Tracked en `.specs/_followups/sprint-2c-google-blocking-function.md`. Riesgo residual documentado: Google self-signup OPEN entre Sprint 2b ship y Sprint 2c ship, pero no exploitable end-to-end sin role assignment manual.
