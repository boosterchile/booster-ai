...
- Date: 2026-05-24
- Status: **Approved** (2026-05-24, PO final approve sobre v3.2 post 4 rondas devils-advocate + 8 decisiones PO; deuda definida tracked en §13 + review.md para resolver en /plan)
- Linked:
...
- [ ] **SC-1.2.2** _(reformado en v3.1 per round 3 P0-R3-1)_: Identity Platform sign-up disabled para AMBOS providers email/password Y Google:
  - **Email/password**: `Sign-in providers → Email/Password → "Allow new accounts to sign up" = OFF`. Verificación: `curl -s -H "Authorization: Bearer $(gcloud auth application-default print-access-token)" -H "x-goog-user-project: booster-ai-494222" "https://identitytoolkit.googleapis.com/admin/v2/projects/booster-ai-494222/config" | jq '.signIn'`.
  - **Google provider**: `Sign-in providers → Google → "Allow new accounts to sign up" = OFF`. Si Identity Platform no expone toggle per-provider (que en algunas configuraciones no soporta), alternativa: backend `app.api/v1/auth/google-callback` rechaza primera sign-in si no hay matching `signup_request` aprobada para el email; user-visible message: "Tu cuenta requiere aprobación de un administrador de Booster".
  - Aplicado via Terraform si provider lo soporta; si no, change manual + captura en `docs/qa/identity-platform-config.md` + TODO IaC. Verificación con jq sobre ambos providers (`.signIn.email` y `.signIn.allowDuplicateEmails` + provider configs).
...
- [ ] **SC-1.2.4**: integration tests cubren: signup-request submit → DB row created → admin approve → user created → user login OK. Y: con self-signup OFF, intento directo de `createUserWithEmailAndPassword` desde cliente → `auth/operation-not-allowed`. Y por cada método inventariado en SC-1.2.0 (`sendPasswordResetEmail`, `signInWithEmailLink`, etc.) un negative test verifica que NO bypassa el admin-approval gate.
...
- [ ] **SC-1.3.2**: wire global en `apps/api/src/main.ts` con `requireNotDemo` aplicado por default a TODOS `POST/PUT/PATCH/DELETE`.
