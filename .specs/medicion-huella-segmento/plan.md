...
> **Línea de método (ADR-077 §4/§5, 2026-09-14):** `lineaMetodoCertificacion` en `packages/carbon-calculator/src/certificacion/linea-metodo.ts` (pura; deriva el nivel con la matriz; vocabulario cerrado con tests de invariantes sobre 3 métodos × 4 fuentes × 7 coberturas). La usan el PDF (fila «Método» en ambas plantillas vía `lineaMetodoCert`, con invariante de ancho A4 a 9 pt), el API (`linea_metodo` + `route_data_source`/`coverage_pct`/`certification_level` en `GET /certificates` y en el detalle de `trip-requests-v2`) y la web (lista de certificados y «Método de cálculo» en el detalle de carga). No existe un segundo vocabulario.
