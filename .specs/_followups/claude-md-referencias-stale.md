...
- **Ítem técnico (`booleanFlag` en `apps/api/src/config.ts`)**: ✅ **convergencia HECHA (2026-06-22)** — el api importa `booleanFlag` de `@booster-ai/config` y se borró la copia local (era byte-idéntica; verificado). typecheck 0, suite api 1548 passed (incl. `config-flags.test.ts`). **Sub-decisión aún ABIERTA (contrato, requiere spec corta)**: "valor no reconocido → default silencioso" vs **fail-closed** (error Zod al startup). NO se cambió el comportamiento — sigue default-silencioso; flippearlo a fail-closed afecta a TODOS los flags y merece su propio spec (un env mal tipeado tumbaría el startup).
