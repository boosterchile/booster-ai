...
5. **Seed T2** — `apps/api/scripts/seed-conductor-e2e.ts` (idempotente: upsert users/empresas; cancela asignaciones E2E viejas **y leftovers activos del conductor T2** que no son el viaje E2E de esta corrida; inserta viaje `asignado` sin Teltonika).
...
7. **CI** — job en `.github/workflows/e2e-pr.yml` (no `ci.yml`): postgres 15 + redis 7 + Auth emulator `:9099` + API `:8080` + preview `:5173`. El check `E2E conductor (Auth emulator + API local)` siempre reporta en PRs a `main`.
